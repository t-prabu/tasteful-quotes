...
function getUpdateColors(swap, callBack) {
    ...
    if (swap) {
        [randColors.a, randColors.b] = [randColors.b, null];
        axios.get('http://api.creativehandles.com/getRandomColor')
            .then(function (response) {
                // handle success
                randColors.b = response.data;
...
function addNewQuote() {
    if (document.querySelector('.js-textarea-input').value.trim().length > 0) {
        ...
        let t = document.createTextNode(document.querySelector('.js-textarea-input').value);
        div.appendChild(t);
        let theParent = document.querySelector('.js-tastful-texts');
